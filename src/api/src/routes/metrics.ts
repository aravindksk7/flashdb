...
router.get('/health', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error(`Error retrieving health metrics: ${error.message}`);
    return res.status(500).json({
      success: false,
      message: error.message
    });
 ...
 */
router.get('/repair', async (_req: Request, res: Response) => {
  try {
    logger.info('Retrieving repair metrics');
    ...
    if (sqlClient) {
      try {
        ...
        const repairsByStatus = statusResult.recordset.map((row: any) => ({
    ...
    logger.error(`Error retrieving repair metrics: ${error.message}`);
    return res.status(500).json({
      success: false,
      message: error.message
    });
...
router.get('/health-trend', async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Retrieving health trend data for ${timeRange}`);
    ...
    if (sqlClient) {
      try {
        ...
        trendData = result.recordset.map((row: any) => ({
    ...
    logger.error(`Error retrieving health trend data: ${error.message}`);
    return res.status(500).json({
      success: false,
      message: error.message
    });
