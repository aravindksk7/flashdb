...
exports.getSecurityConfig = exports.sensitiveFields = exports.requestIdMiddleware = exports.requestValidationMiddleware = exports.httpsEnforcementMiddleware = exports.securityHeadersMiddleware = exports.rateLimitMiddleware = exports.corsOptions = void 0;
    ...
    if (req.method !== 'OPTIONS') {
        ...
        if (['POST', 'PUT', 'PATCH'].includes(req.method)) {
            ...
            if (contentType && !contentType.includes('application/json')) {
                logger_1.default.warn(`Invalid Content-Type: ${contentType}`);
                res.status(400).json({
                    success: false,
        ...
        for (const [key, value] of Object.entries(params)) {
            ...
            if (typeof value === 'string' && suspiciousPatterns.test(value)) {
                logger_1.default.warn(`Suspicious pattern detected in ${key}: ${value.substring(0, 50)}`);
                res.status(400).json({
                    success: false,
...
function redactSensitiveData(obj) {
    ...
    for (const key in result) {
        if (exports.sensitiveFields.some(field => key.toLowerCase().includes(field.toLowerCase()))) {
            result[key] = '[REDACTED]';
        }
